---
title: "PrimerPaper_FOUILLOUX2024"
output:
...
Let's load some data; these are ordered by chunks. 

1. Here, we have the ultimate ddPCR run by Eric and Chloe ran on July 24th, 2024. This is the entire run of the ddPCR plate (ddPCR_fullrun).

2-6. These are WELL SPECIFIC data. It means we see the actual amplification for **EACH** and every droplet in each well that is summarized in the full run. Cool huh?

7-9. These are WELL SPECIFIC DATA for field samples that we ran as well.
...
#Generate a table for the summary statistics of each of the dilations
multiplex_standard %>%
...
Remember here that Dilution Factor "8" does not mean 10^-8, but was just used to place an actual lakeat the farthest end of the x-axis. Here, dilution 2-6 is accurate (10^-2 to 10^-6); position 7 refers to the NTC and "8" is a field sample.
...
```{r Wild Zooplankton Tows, "Infection" rates}
...
cope = (340955)/100 #this is based our gene copy estimate from our standard
worm = (159857)/100 #this is based our gene copy estimate from our standard

wildinfection%>%
group_by(Infec, Species, Dilution, Ten) %>%
#NOTE THAT HERE I AM CORRECTING FOR THE DILUTION FACTOR, SO ALL NUMBERS IN THE TABLE ARE CORRECTED.
mutate(corr = case_when(Ten == "1" ~ Concentration *10,
...
summarise(mean = mean(corr),
          ...
          se = sd / sqrt(n), 
          worms = mean/worm, 
          #remember, "worm" is how many copies/uL there is of gene for 1 estimate worm
          #be careful here, it will give you values everything but just look at worms
          #remember, "cope" is how many copies/uL there is of gene for 1 estimate copepod
          copes = mean/cope) #be careful here, it will give you values for both but just look at copes

## From summary table ####
#(just taking 10^-1 estimates (corrected) for worms (more DNA, more accurate), 
#and 10^-2 for copepods (because they max-out at 10^-1))

#PCH JUNE = 31.9 copepods, 0.00813 schisto
0.00813/31.9

#BLA MARCH = 60.2 copepods, 0.0423 schisto
0.423/60.2
...
#DNA quantification
DNA concentration is estimated bymodeling a Poisson distribution. The formula used for Poisson modelling is

Copies per droplet = -ln(1-p)
where p = fraction of positive droplets.

*Question: How much DNA is in 100 copepods?*
...
The best solution may be to simply average between all of the standards?
...
*Question: How much DNA is in 100 S. solidus?*
Here we are looking at DNA from both singplex and multiplex reactions. Unfortunately, we only have this single standard (which took over 4 months to make), so our estimates are confined to single standard.

Here, the assay repeatability was determined by the % coefficient
of variation (%CV = concentration standard deviation/concentration
mean * 100) between the replicates. (from Hiillos et al. 2021)

```{r 100 S. solidus, Simple Stats, CV}
schisto %>%
        filter(Species == "Schisto") %>%
#don't need to do treatment as we already showed that no difference between single and multiplex
